...
LeanRows separates practical 0.1 release qualification from longer assurance
...
| APP-01 | The native window, owner-data list, status bar, accessibility names, keyboard focus, system colors, and cleanup pass the shell smoke. | `leanrows.exe --smoke-test` in the Windows workflow |
...
| A11Y-01 | Automated native smoke confirms accessible names, roles, focus, system colors, and high-contrast queries. The documented keyboard path is manually usable before tagging. | Native smoke and release checklist |
...
they are not represented as completed 0.1 release gates:
...
A 0.1 tag requires every item in the release table to pass or to have a written,
publicly reviewed exception that narrows the release claim. A failed check is
fixed or the release scope changes; it is never relabeled as a pass. Final ZIP
digests and host-specific memory results are recorded only after the source and
binary are frozen.
